...
We got _challengeDNS_anon.pcapng_ file, and if we open it with Wireshark and apply **dns** filter we can see that first packet has name ```init.OBLWI5KNOBDDC3DFFZTGYNDHPQZQ.base32.dnsresearch.ml```
...
To solve the second part, we need to go back to description of part 1, and find out how DNS exfiltration works. We can find source code of [DNSExfiltrator tool]("https://github.com/Arno0x/DNSExfiltrator") on github:
...
Tshark is a terminal oriented version of Wireshark, so we can also use this tool to extract data from file:
```sh
